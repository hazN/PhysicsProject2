	...
	std::string& errorText)
{
	...
	for (unsigned int count = 0; count != modelDrawInfo.numberOfTriangles; count++)
	{
		...
		::sTrianglePLY* triangle = new ::sTrianglePLY;
		triangle->vertexIndices[0] = pTheModelTriangleArray[count].vertexIndices[0];
		triangle->vertexIndices[1] = pTheModelTriangleArray[count].vertexIndices[1];
		triangle->vertexIndices[2] = pTheModelTriangleArray[count].vertexIndices[2];
		modelDrawInfo.pTriangles.push_back(triangle);
	}
	...
	for (unsigned int index = 0; index != modelDrawInfo.numberOfVertices; index++)
	{
		...
		//if (modelDrawInfo.meshName != "Warrior" && modelDrawInfo.meshName != "ISO_Sphere_1" && modelDrawInfo.meshName != "ISO_Sphere_2")
		//{
		//    terrainVertices.push_back(new glm::vec3(pTheModelArray[index].x, pTheModelArray[index].y, pTheModelArray[index].z));
		//}
	}
